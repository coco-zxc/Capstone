...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Mission Outcomes on All Sites')
        return fig
    else:
        filtered_df=spacex_df[spacex_df['Launch Site']== entered_site]
        filtered_df=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig=px.pie(filtered_df,values='class count',
                        names='class',title="Total Success Launches for site")
        return fig
# TASK 4:
...
def get_scatter_chart(site_dropdown,payload_slider):
    ...
    if site_dropdown == 'ALL':
        upper_filter_slider=spacex_df[['Payload Mass (kg)']]<high_value
        lower_filter_slider=spacex_df[['Payload Mass (kg)']]>low_value


        filter_slider=upper_filter_slider&lower_filter_slider
        filter_slider=filter_slider['Payload Mass (kg)']

        data=spacex_df[filter_slider]
